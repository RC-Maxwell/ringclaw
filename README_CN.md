...
RingClaw 支持双客户端架构：**Private App** 用于读取消息，**Bot App** 用于在群聊中回复。

```mermaid
graph LR
    RC[RingCentral] -->|WebSocket| PrivateApp[Private App]
    PrivateApp -->|读取消息| RingClaw
    RingClaw -->|在私聊/允许的群中回复| BotApp[Bot App]
    RingClaw -->|在其他地方回复| PrivateApp
```

**为什么使用 Bot？** Private App 的消息显示为安装者的身份。Bot App 有独立的身份、头像和名称，能清楚地区分哪些回复来自 AI。

### 配置

1. 在 [RingCentral 开发者门户](https://developers.ringcentral.com/) 创建 **Bot App**
2. 安装到账号并复制 Bot Token
3. 添加到配置：

```json
{
  "ringcentral": {
    "chat_ids": ["群聊-id-1", "群聊-id-2"],
